...
object SchemaFlattener {
   ...
   */
  private[ddlast] case class SubSchema(elems: Map[String, Map[String, String]], required: Set[String])

  /**
   * Helper classes to extract info from JSON Schema's JValue
   */
  private[ddlast] abstract sealed trait Info

  /**
   * Helper class for all primitive types
   */
  private[ddlast] case object PrimitiveInfo extends Info

  /**
   * Helper class for arrays
   */
  private[ddlast] case object ArrayInfo extends Info
   ...
   */
  private[ddlast] case class ObjectInfo(properties: List[JField], required: Set[String]) extends Info

  /**
   * Helper object extracted from JSON Schema's object which should be
   * represented as VARCHAR(4096) in future
   */
  private[ddlast] case object FlattenObjectInfo extends Info
   ...
   * @return a validated map of keys and attributes or a failure string
   */
  private[ddlast] def processProperties(propertyList: List[JField],
   ...
   */
  @tailrec
  private[ddlast] def processAttributes(attributes: List[JField], accum: Map[String, String] = Map()): Validation[String, Map[String, String]] =
    attributes match {
      case x :: xs => {
        x match {
   ...
   */
  private[ddlast] def stringifyArray(list: List[JValue], accum: String = "", delim: String = ","): Validation[String, String] =
    list match {
      case x :: xs => {
        x match {
   ...
   */
  private[ddlast] def getRequiredProperties(jObject: Map[String, JValue]): Validation[String, List[String]] = {
   ...
   *         with the element
   */
  private[ddlast] def getElemInfo(maybeAttrList: List[JField]): Validation[String, Info] = {
   ...
   */
  private[ddlast] def getElemType(types: JValue): Validation[String, String] = {
   ...
   */
  private[ddlast] def isProductType(types: String): Boolean =
   ...
   */
  private[ddlast] def isNullable(types: String): Boolean =
   ...
   */
  private[ddlast] def extractTypesFromProductType(types: String): List[String] =
   ...
   */
  private[ddlast] def updateType(properties: Map[String, String], currentType: String): Map[String, String] =
   ...
   *         alphabetical order.
   */
  private[ddlast] def getOrderedMap(paths: Map[String, Map[String, String]]): ListMap[String, Map[String, String]] =
