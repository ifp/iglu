...
import Ddl.Column
import RedshiftDdlGenerator.{ selfDescSchemaColumns, parentageColumns }
...
object JsonPathGenerator {
  ...
  def getJsonPathsFile(columns: List[Column], rawMode: Boolean = false): String = {
    val columnNames: List[String] =
      if (rawMode) {   // everything is data in raw mode
        prependStrings(columns.map(_.columnName), JsonPathPrefix.Data)
        ...
        val schemaFieldList    = prependStrings(JsonPathSchemaFields, JsonPathPrefix.Schema)
        val hierarchyFieldList = prependStrings(JsonPathHierarchyFields, JsonPathPrefix.Hierarchy)
        val dataFieldList      = prependStrings(dataColumns, JsonPathPrefix.Data)
   ...
   */
  private[ddlast] def formatFields(fields: List[String]): List[String] = {
    val prefix = getWhiteSpace(8)
    for {
      field <- fields
    } yield {
